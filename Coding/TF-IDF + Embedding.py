...
# Function to retrieve top 10 documents based on combined TF-IDF and Embedding scores
def retrieve_top_10_combined(query_tokens, tfidf_vectorizer, tfidf_doc_matrix, documents_df, model, weights):
    # TF-IDF Scores
    ...
    # Combined Scores
    combined_scores = weights['tfidf'] * tfidf_scores + weights['embedding'] * embedding_scores
...
def evaluate_map(queries_df, ground_truth, tfidf_vectorizer, tfidf_doc_matrix, documents_df, model, weights):
    mean_average_precision = 0.0
    for _, row in queries_df.iterrows():
        query_id = row['query_id']
        query_tokens = row['processed_tokens']
        relevant_docs = ground_truth.get(query_id, [])
        ranked_docs = retrieve_top_10_combined(query_tokens, tfidf_vectorizer, tfidf_doc_matrix, documents_df, model, weights)
...
# Define weights for TF-IDF and Embedding
weights = {'tfidf': 0.5, 'embedding': 0.5}

# Calculate MAP for combined TF-IDF + Embedding
map_score = evaluate_map(queries_df, ground_truth, tfidf_vectorizer, tfidf_doc_matrix, documents_df, fasttext_model, weights)
print(f"Mean Average Precision (MAP) for Combined TF-IDF + Embedding: {map_score:.4f}")
